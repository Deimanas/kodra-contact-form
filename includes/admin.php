...
function kcf_admin_messages_page(){ if(!current_user_can('manage_options')) return; global $wpdb; $table=KCF_TABLE; $highlight_script='<script>(function(){document.addEventListener("DOMContentLoaded",function(){var highlightEls=document.querySelectorAll(".kcf-highlight");if(!highlightEls.length)return;setTimeout(function(){highlightEls.forEach(function(el){el.classList.add("kcf-fade-end");setTimeout(function(){el.classList.remove("kcf-fade-end");el.classList.remove("kcf-highlight");},800);});},60000);});})();</script>';
  ...
  if(isset($_GET['view'])){ $id=intval($_GET['view']); $row=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$table} WHERE id=%d",$id),ARRAY_A); echo '<h2>#'.intval($id).'</h2>'; if($row){ $layout=kcf_get_form_layout(); $indexed=kcf_index_form_fields($layout); echo '<div class="kcf-two-col"><div class="kcf-col kcf-col-left"><h3>Pagrindinė informacija</h3><table class="widefat striped">'; $primary_keys=['vardas'=>['value'=>$row['name'],'is_textarea'=>false],'telefonas'=>['value'=>$row['phone'],'is_textarea'=>false],'email'=>['value'=>$row['email'],'is_textarea'=>false],'zinute'=>['value'=>$row['message'],'is_textarea'=>true]]; foreach($primary_keys as $key=>$info){ $field=isset($indexed[$key])?$indexed[$key]:['key'=>$key]; $label_raw=kcf_format_field_label($field,false); $label=kcf_strip_required_marker($label_raw); if($label===''){ $label=ucfirst($key); } $value=(string)$info['value']; $cell=$info['is_textarea']?wp_kses_post(nl2br(esc_html($value))):esc_html($value); echo '<tr><th>'.esc_html($label).'</th><td>'.$cell.'</td></tr>'; } echo '</table>'; $extra=kcf_decode_message_extra($row['extra']??''); if(!empty($extra)){ usort($extra,function($a,$b)use($indexed){ $orderA=isset($indexed[$a['key']]['order'])?intval($indexed[$a['key']]['order']):0; $orderB=isset($indexed[$b['key']]['order'])?intval($indexed[$b['key']]['order']):0; if($orderA===$orderB){ return strcmp($a['key'],$b['key']); } return $orderA<=>$orderB; }); echo '<h3>Papildomi laukai</h3><table class="widefat striped">'; foreach($extra as $item){ $extra_label=$item['label']!==''?$item['label']:''; if($extra_label===''){ if(isset($indexed[$item['key']])){ $extra_label=kcf_strip_required_marker(kcf_format_field_label($indexed[$item['key']],false)); } else { $extra_label=$item['key']; } } $extra_value=is_string($item['value'])?$item['value']:''; if($extra_value===''){ continue; } $extra_type=strtolower($item['type']??''); $extra_cell=$extra_type==='textarea'?wp_kses_post(nl2br(esc_html($extra_value))):esc_html($extra_value); echo '<tr><th>'.esc_html($extra_label).'</th><td>'.$extra_cell.'</td></tr>'; } echo '</table>'; } echo '</div><div class="kcf-col kcf-col-right"><h3>Kita</h3><table class="widefat striped">'; $company_field=isset($indexed['imone'])?$indexed['imone']:['key'=>'imone']; $company_label=kcf_strip_required_marker(kcf_format_field_label($company_field,false)); if($company_label==='') $company_label='Įmonė'; foreach(['Data'=>$row['created_at'],$company_label=>$row['company'],'IP adresas'=>$row['ip'],'Naršyklė'=>$row['user_agent'],'Įrašo ID'=>$row['id']] as $l=>$v){ echo '<tr><th>'.esc_html($l).'</th><td>'.wp_kses_post(nl2br(esc_html((string)$v))).'</td></tr>'; } echo '</table></div></div>'; echo '<style>.kcf-two-col{display:grid;grid-template-columns:7fr 3fr;gap:18px;max-width:100%;width:100%}.kcf-two-col th{width:220px}.kcf-two-col th,.kcf-two-col td{word-break:break-word;overflow-wrap:anywhere}.kcf-thread-item.kcf-latest strong{font-weight:500}.kcf-unread-dot{display:inline-block;width:8px;height:8px;background:#d63638;border-radius:50%;margin-left:6px;vertical-align:middle}</style>';
    ...
    if($replies){
      ...
      echo '<div class="kcf-thread-layout">';
      echo '<div class="kcf-thread-controls"><div class="kcf-thread-bulk-actions"><label><input type="checkbox" class="kcf-thread-select-all" aria-label="Pažymėti visus įrašus"> Pažymėti visus</label><button type="submit" class="button button-small button-danger kcf-thread-bulk-delete" disabled>Trinti pažymėtus</button></div></div>';
      echo '<div class="kcf-thread-list">';
      ...
      foreach($replies as $rep){
        ...
        $header.='<div class="kcf-thread-title"><strong>'.esc_html($subject_display).'</strong></div>';
  ...
  $layout_labels=kcf_get_form_layout(); $indexed_labels=kcf_index_form_fields($layout_labels);
  $name_label=kcf_strip_required_marker(kcf_format_field_label($indexed_labels['vardas']??['key'=>'vardas'],false)); if($name_label==='') $name_label='Vardas';
  $email_label=kcf_strip_required_marker(kcf_format_field_label($indexed_labels['email']??['key'=>'email'],false)); if($email_label==='') $email_label='El. paštas';
  $phone_label=kcf_strip_required_marker(kcf_format_field_label($indexed_labels['telefonas']??['key'=>'telefonas'],false)); if($phone_label==='') $phone_label='Telefonas';
  $company_label=kcf_strip_required_marker(kcf_format_field_label($indexed_labels['imone']??['key'=>'imone'],false)); if($company_label==='') $company_label='Įmonė';
  $message_label=kcf_strip_required_marker(kcf_format_field_label($indexed_labels['zinute']??['key'=>'zinute'],false)); if($message_label==='') $message_label='Žinutė';
  ...
  echo '<table class="widefat striped" style="table-layout:fixed"><thead><tr><td id="cb" class="manage-column column-cb check-column"><input type="checkbox" class="kcf-select-all" aria-label="Pažymėti visus"></td><th style="width:60px">ID</th><th style="width:160px">Data</th><th style="width:140px">'.esc_html($name_label).'</th><th style="width:220px">'.esc_html($email_label).'</th><th style="width:160px">'.esc_html($phone_label).'</th><th style="width:160px">'.esc_html($company_label).'</th><th>'.esc_html($message_label).'</th><th style="width:190px">Veiksmai</th></tr></thead><tbody>';
  if($items){
    ...
    if($row){ $thread_root=intval($row['thread_root'])>0?intval($row['thread_root']):intval($row['id']);
      ...
      if($thread){
        ...
        if($has_inbound){
          ...
          echo '<div class="kcf-thread-layout">';
          echo '<div class="kcf-thread-controls"><div class="kcf-thread-bulk-actions"><label><input type="checkbox" class="kcf-thread-select-all" aria-label="Pažymėti visus laiškus"> Pažymėti visus</label><button type="submit" class="button button-small button-danger kcf-thread-bulk-delete" disabled>Trinti pažymėtus</button></div></div>';
          echo '<div class="kcf-thread-list">';
        } else {
          echo '<div class="kcf-thread-list">';
        }
        ...
        foreach($thread as $item){
          ...
          if($has_inbound){
            $header='<div class="kcf-thread-header">';
            $header.='<div class="kcf-thread-title"><strong>'.$header_subject.'</strong></div>';
            if(intval($item['direction'])===1){
